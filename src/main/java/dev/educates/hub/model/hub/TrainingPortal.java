...
import dev.educates.hub.model.educates.AuthResponse;
import dev.educates.hub.model.educates.EducatesCatalog;
import dev.educates.hub.model.educates.EducatesSessionResponse;
...
public class TrainingPortal {
    ...
    private void internalUpdateInfo(int retries) {
        ...
        authenticate();
        if (portalAuth != null) {
            EducatesCatalog EducatesCatalog = null;
            try {
                // If we have a valid Token
                EducatesCatalog = this.webClient.get()
                        .uri(EducatesPortalConfig.CATALOG_ENDPOINT)
                        .headers(headers -> headers.setBearerAuth(portalAuth.getAccessToken()))
                        .retrieve().bodyToMono(EducatesCatalog.class).block();
                if (EducatesCatalog != null) {
                    EducatesCatalog.getEnvironments().forEach(educatesEnv -> {
                        WorkshopEnvironment we = new WorkshopEnvironment(educatesEnv);
            ...
            logger.error("Not possible tp query portal({}) at ({}) due to lack of authentication credentials", name,
                    url);
            return;
        }
    }
    ...
    private void authenticateNew() {
        ...
        try {
            ...
            AuthResponse response = this.webClient.post()
                    .uri(EducatesPortalConfig.OAUTH_ENDPOINT)
            ...
            logger.error("Have you verified if the robot credentials are correct for this portal({}) at ({})", name,
                    url);
        }
    }
    ...
    public String startWorkshop(String workshopName, String callbackUrl, int retries) {
        ...
        if (portalAuth != null) {
            ...
            try {
                EducatesSessionResponse response = this.webClient.get()
                        .uri(uriBuilder -> uriBuilder.path("/workshops/environment/{workshopName}/request/")
                                .queryParam("index_url", callbackUrl)
                                .build(workshopName))
                        .headers(headers -> headers.setBearerAuth(portalAuth.getAccessToken()))
                        .retrieve().bodyToMono(EducatesSessionResponse.class).block();

                logger.info("Started session '{}' at url '{}'", response.getSession(), response.getUrl());
                return this.getUrl() + response.getUrl();
            } catch (WebClientResponseException e) {
                portalAuth = null;
                logger.error("Error ({} - {}) while starting workshop({})", e.getRawStatusCode(), e.getStatusText(),
                        workshopName);
                if (retries-- > 0) {
            ...
            logger.error(
                    "Not possible to execute startWorksho action as we don't have valid credentials for workshop {}",
                    workshopName);
            return null;
        }
    }
     ...
     * @param workshops
     */
    public void getWorkshopIDInfo(Map<String, List<WorkshopLocation>> workshops) {
        environments.values().forEach(env -> {
            String uid = env.getWorkshop().getId();
    ...
    public boolean equals(Object o) {
        ...
        return Objects.equals(name, trainingPortal.name) && Objects.equals(url, trainingPortal.url)
                && Objects.equals(robot, trainingPortal.robot)
                && Objects.equals(environments, trainingPortal.environments);
    }
